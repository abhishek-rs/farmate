...
HPJ = HPJ-1 + RFJ + IRJ - ETJ - DPJ - ROJ  			(1)

HPJ = is the ponding depth (water height) on day J, given in cm
RFJ = is the rainfall on day J, in cm
IRJ = is the irrigation applied on day J, in cm
ETJ = Is the crop evotranspiration (cm), this is both the water that evaporated as well as water taken in by the crop. 
DPJ = Depth percolation on day J, the water that is lost through the ground (cm)
ROJ = Run Off, surface runoff on day J (cm)
...
4. Main Dashboard - This is our key view which enables the user to monitor his fields, look at neighbouring fields and see useful data about them. The view mainly consists of three parts.
...
Update modal - Besides the above views we also have a modal to allow users to update irrigation done on previous days or correct field water level if the information being displayed by the service is incorrect.  
...
What else? EIT? Aalto? Life? Nobody?
Maybe telegram for cat gifs of Telegram and EIT for not locking us out of CLC for the summer?
References
